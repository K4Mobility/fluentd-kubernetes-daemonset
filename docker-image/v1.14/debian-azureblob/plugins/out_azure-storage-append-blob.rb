  ...
  module Plugin
    class AzureStorageAppendBlobOut < Output
      ...
      def generate_log_name(chunk, index)
        ...
        year = Time.at(metadata.timekey).utc.strftime("%Y")
        month = Time.at(metadata.timekey).utc.strftime("%m")
        day = Time.at(metadata.timekey).utc.strftime("%d")
        hour = Time.at(metadata.timekey).utc.strftime("%H")
